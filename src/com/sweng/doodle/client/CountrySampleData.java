...
public class CountrySampleData {  
	...
	public static ListGridRecord createRecord(String id, String nome, String luogo, String descrizione, Date dal, Date al, String check) {  
	...
	public static RecordList setEventi(LinkedList<Evento> eventi){
		...
		for(int i = 0 ; i < eventi.size(); i++){
			dataTable.add(createRecord(eventi.get(i).getID(), eventi.get(i).getNome(), eventi.get(i).getLuogo(), eventi.get(i).getDesc(), eventi.get(i).getDal(), eventi.get(i).getAl(),eventi.get(i).getCheck() == 0 ? "chiuso" :  "aperto"));
		}
		return dataTable;
	}
